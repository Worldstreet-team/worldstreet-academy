...
import { BookOpenIcon, GraduationCapIcon, StarIcon } from "lucide-react"
    ...
    return (
      <div className="flex flex-col items-center justify-center py-10 text-center">
        <div className="p-4 rounded-lg bg-accent/20 mb-4">
          <GraduationCapIcon  size={28} className="text-muted-foreground/50" />
        </div>
    ...
    <div className="space-y-3">
      {items.slice(0, 12).map((course: CourseItem, i: number) => (
        <motion.div
          ...
          className="rounded-lg bg-card/50 border border-ws-hairline
        ...
        >
          ...
          {course.thumbnailUrl ? (
            <div className="relative w-full aspect-2/1 bg-muted overflow-hidden">
              <Image
                ...
                alt={course.title}
                fill
                className="object-cover transition-transform duration-[var(--ws-motion-slow)] ease-out"
                sizes="400px"
              />
              {/* Price badge — top right */}
              <div className="absolute top-2.5 right-2.5">
                {course.pricing === "free" ? (
                  <span className="text-[11px] font-semibold px-2.5 py-1 rounded-lg bg-ws-success/90 text-white">
                    Free
                  </span>
                ) : course.price ? (
                  <span className="text-[11px] font-semibold px-2.5 py-1 rounded-lg bg-background/80 text-foreground border border-ws-hairline">
                    ${course.price}
                  </span>
                ) : null}
              </div>
              ...
              {course.level && (
                <div className="absolute top-2.5 left-2.5">
                  <span className="text-[11px] font-medium px-2.5 py-1 rounded-lg bg-background/70 text-foreground/80 border border-ws-hairline capitalize">
                    {course.level}
                  </span>
                </div>
              )}
            ...
            <div className="w-full h-20 bg-accent/20 flex items-center justify-center">
              <GraduationCapIcon  size={28} className="text-muted-foreground/30" />
            </div>
          )}
          ...
          <div className="p-3.5 space-y-2.5">
            ...
            <div className="flex items-center gap-3 text-[11px] text-muted-foreground">
              {(course.rating ?? 0) > 0 && (
                <span className="flex items-center gap-0.5 text-ws-warning">
