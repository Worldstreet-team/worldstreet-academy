...
- **D15 — The wizard is removed; the journey is the blueprint's (owner, 2026-09-18).** The `/dashboard/start` "Step 1 of 3" picker is gone, and no "Step N of M" wizard replaces it. The journey is blueprint §17/§18: **Homepage → School → Program → Package → Checkout** — a school chip or card opens `/schools/<slug>` (the school and its programs), a program opens `/programs/<slug>`, where the learner chooses Basic, Standard or Executive with each package's breakdown, and "Continue to payment" opens `/dashboard/checkout?courseId=<id>&package=<key>`. Checkout shows the chosen package as a summary with a "Change" link back to the program page; it no longer offers a package picker. The gate (D13) now sends a `USER` with no enrollment and no intent to `/schools` (or `/schools/<slug>` when the landing's `wsa_school` cookie names a school). `/dashboard/start` survives only as a redirect for old links (`?school=<slug>` → `/schools/<slug>`, `?pick=1` → `/schools`, a bare visit follows the cookie). With the wizard's school-only save gone, an `EnrollmentIntent` is written only by checkout (reaching it saves the choice; "Save and pay later" and the wallet-funding round trip wait for that save). This supersedes D11's picker steps; D11's "a school has no price" stands.
...
- [x] "Save and pay later" on the package step and on checkout (the package step is retired by D15; checkout keeps it); dashboard hero offers the order back; emails at 24 h and 72 h, at most once each, stopping on enrolment.
